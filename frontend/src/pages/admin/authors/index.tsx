                ...
                <tbody className="divide-y divide-gray-100">
                  {authors.map((author: any) => (
                    <tr key={author.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      `}>
                          {author.status === 'ACTIVE' ? <CheckCircle2 className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" strokeWidth={2.5} />}
                          {author.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => openDrawer(author)} className="p-2 text-saffron hover:text-orange-600 hover:bg-saffron/10 rounded-md transition-colors">
          ...
          <div className="relative w-full max-w-4xl bg-gray-50 h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="flex items-center justify-between px-6 py-4 bg-orange-100 border-b border-orange-200">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-xl font-bold tracking-wide text-slate-900 uppercase">{editingId ? 'Edit Author' : 'Create Author'}</h1>
                </div>
              </div>
              <button
                type="submit"
