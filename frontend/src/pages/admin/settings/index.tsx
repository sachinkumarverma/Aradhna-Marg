  ...
  return (
    <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-5 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="md:col-span-2">
          <Field label="Address">
            <Textarea {...register('contactAddress')} rows={2} />
          </Field>
        </div>
      </div>
      <SaveButton isPending={mutation.isPending} />
      ...
      <div className="grid grid-cols-1 gap-5 max-w-lg">
        <Field label="Channel ID">
          <Input
            ...
            className="w-full px-3 py-2 border rounded-md bg-white font-mono text-sm focus:ring-2 focus:ring-saffron/20 focus:border-saffron outline-none"
          />
        </Field>
        <Field label="Channel URL">
          <Input {...register('youtubeChannelUrl')} />
        </Field>
          ...
          <div className="pt-2 border-t border-gray-200 space-y-2">
            <p className="text-sm mt-4 text-gray-500">
              ...
              <strong className="text-gray-900">
                {defaults?.youtubeLastSync
                  ? format(new Date(defaults.youtubeLastSync), 'dd MMM yyyy, hh:mm a')
                  : 'Never'}
              </strong>
            </p>
...
const SeoSection = ({ defaults }: { defaults: any }) => {
  ...
  return (
    <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-5 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div className="md:col-span-2">
          <Field label="Global Site Title">
            <Input {...register('seoSiteTitle')} />
          </Field>
        </div>
        <div className="md:col-span-2">
          <Field label="Global Meta Description">
            <Textarea {...register('seoMetaDescription')} rows={3} />
          </Field>
        </div>
        <div className="md:col-span-2">
          <Field label="Meta Keywords">
            <Input {...register('seoMetaKeywords')} />
          </Field>
        </div>
      </div>
      <SaveButton isPending={mutation.isPending} />
...
const AdvertisementSection = ({ defaults }: { defaults: any }) => {
  ...
  const { register, handleSubmit } = useForm({ values: defaults });
  const AdTextarea = React.forwardRef<HTMLTextAreaElement, React.TextareaHTMLAttributes<HTMLTextAreaElement>>(
    (props, ref) => {
      const [isLocked, setIsLocked] = useState(true);
      return (
        <div className="relative group">
  ...
  return (
    <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-5 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 gap-5">
        <Field label="Top Banner Ad Code">
          <AdTextarea {...register('adTopBanner')} rows={3} />
        </Field>
        <Field label="Inline Ad Code">
          <AdTextarea {...register('adInline')} rows={3} />
        </Field>
        <Field label="Sidebar Ad Code">
          <AdTextarea {...register('adSidebar')} rows={3} />
        </Field>
        <Field label="Footer Ad Code">
          <AdTextarea {...register('adFooter')} rows={3} />
        </Field>
      </div>
      <SaveButton isPending={mutation.isPending} />
    </form>
