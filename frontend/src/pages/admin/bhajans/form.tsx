          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-wide text-slate-900 uppercase">
                {isEditing ? 'Edit Bhajan' : 'Create Bhajan'}
              </h1>
              {lastSaved && (
                <p className="text-xs text-green-600 font-medium mt-1">Last saved: {lastSaved.toLocaleTimeString()}</p>
              )}
            </div>
          </div>
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-md shadow-sm border border-blue-100 p-6 space-y-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="w-full flex flex-col">
                      <ImageUploadWithCrop
                        ...
                        onChange={(val) => setValue('image_url', val, { shouldValidate: true, shouldDirty: true })}
                        onRemove={() => setValue('image_url', '', { shouldValidate: true, shouldDirty: true })}
                        aspectRatio={16 / 9}
                        shape="rect"
                  ...
                  <div className="space-y-1.5 mt-4">
                    <label className="text-sm font-semibold text-gray-800">English Lyrics (Optional)</label>
                    <Controller
                      name="lyrics_english"
                      control={control}
                      render={({ field }) => (
                  ...
                  <div className="space-y-3">
                    ...
                    {videoSourceMode === 'automatic' ? (
                      <div className="bg-blue-50/50 p-3 rounded-md border border-blue-100 text-xs text-blue-700">
                        The primary YouTube video will automatically be linked based on title matching during the sync
                        process.
                      </div>
                    ) : (
                      <div className="space-y-1.5 mt-4">
                        ...
                        />
                        {errors.original_youtube_url && (
                          <p className="text-xs text-red-500">{errors.original_youtube_url.message as string}</p>
                        )}
                      </div>
                    )}
                  </div>
