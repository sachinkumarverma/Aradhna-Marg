...
class SearchRepository extends base_repository_1.BaseRepository {
    constructor() {
        super('bhajans');
    }
    /**
    ...
    async searchFTS(options) {
        ...
        if (filters?.hasPdf)
            dbQuery = dbQuery.eq('has_pdf', true);
        if (filters?.hasVideo)
            dbQuery = dbQuery.not('youtube_video_id', 'is', null);
        // Apply Sorting
        switch (sort) {
            case 'NEWEST':
    ...
    async getTrendingSearches() {
        ...
        return data.map((row) => row.search_query);
    }
}
