...
class CategoryController {
    ...
    async createCategory(req, res, next) {
        try {
            const validatedData = CategoryValidator_1.createCategorySchema.parse(req.body);
    ...
    async updateCategory(req, res, next) {
        try {
            const validatedData = CategoryValidator_1.updateCategorySchema.parse(req.body);
    ...
    async bulkEditCategories(req, res, next) {
        try {
            ...
            if (!Array.isArray(ids))
                throw new appError_1.AppError('ids must be an array', 400);
            const validatedData = CategoryValidator_1.updateCategorySchema.parse(data);
