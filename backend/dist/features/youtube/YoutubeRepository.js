...
class YoutubeVideoRepository {
    ...
    async getVideos(search, status, type, sortBy = 'published_at', sortOrder = 'desc', page = 1, limit = 20) {
        if (type) {
            ...
            const result = await DatabaseClient_1.db.query(allDataQuery, params);
    ...
    async getStats() {
        try {
            ...
            const results = await Promise.all(queries.map(q => DatabaseClient_1.db.query(q)));
            return {
    ...
    async upsertVideos(videos) {
        ...
        try {
            const existingResult = await DatabaseClient_1.db.query(`SELECT youtube_video_id FROM ${this.tableName} WHERE youtube_video_id IN (${placeholders})`, incomingIds);
      ...
      `;
            ...
            await DatabaseClient_1.db.query(query, params);
        }
        return {
            imported: newCount,
            updated: updateCount
    ...
    async getSyncHistory() {
        try {
            const result = await DatabaseClient_1.db.query(`SELECT * FROM youtube_sync_logs ORDER BY started_at DESC LIMIT 50`);
            return result.rows;
        }
        catch (error) {
            if (error.code === '42P01' || error.message?.includes('does not exist'))
    ...
    async logSync(channelId, status, errorMessage) {
        try {
            await DatabaseClient_1.db.query(`INSERT INTO youtube_sync_logs (channel_id, status, error_message, completed_at) VALUES ($1, $2, $3, NOW())`, [channelId, status, errorMessage]);
        }
        catch (e) {
    ...
    async linkBhajan(videoId, bhajanId) {
        await DatabaseClient_1.db.query(`UPDATE ${this.tableName} SET linked_bhajan_id = $1, import_status = $2 WHERE id = $3`, [bhajanId, bhajanId ? 'LINKED' : 'NEW', videoId]);
        return { success: true };
    }
    async updateStatus(videoId, status) {
        await DatabaseClient_1.db.query(`UPDATE ${this.tableName} SET import_status = $1 WHERE id = $2`, [status, videoId]);
        return { success: true };
    }
    async deleteVideo(videoId) {
        await DatabaseClient_1.db.query(`DELETE FROM ${this.tableName} WHERE id = $1`, [videoId]);
        return { success: true };
    }
}
