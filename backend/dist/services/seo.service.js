...
class SeoService {
    async getOverview() {
        ...
        for (const table of tables) {
            // Get totals
            let totalQuery = supabase_1.supabase.from(table).select('*', { count: 'exact', head: true });
            if (table === 'bhajans')
                totalQuery = totalQuery.is('youtube_video_id', null);
            const { count: total } = await totalQuery;
            // Get missing titles
            let missingTitleQuery = supabase_1.supabase.from(table).select('*', { count: 'exact', head: true }).or('seo_title.is.null,seo_title.eq.');
            if (table === 'bhajans')
                missingTitleQuery = missingTitleQuery.is('youtube_video_id', null);
            const { count: missingTitle } = await missingTitleQuery;
            // Get missing descriptions
            let missingDescQuery = supabase_1.supabase.from(table).select('*', { count: 'exact', head: true }).or('meta_description.is.null,meta_description.eq.');
            if (table === 'bhajans')
                missingDescQuery = missingDescQuery.is('youtube_video_id', null);
            ...
            if (table === 'bhajans')
                overview.totalBhajans = total || 0;
            if (table === 'articles')
                overview.totalArticles = total || 0;
            if (table === 'festivals')
                overview.totalFestivals = total || 0;
            if (table === 'puranas')
                overview.totalPuranas = total || 0;
    ...
    async getIssues() {
        ...
        const issues = [];
        for (const table of tables) {
            let missingTitlesQuery = supabase_1.supabase
                .from(table)
                .select('id, title')
            ...
            let missingDescsQuery = supabase_1.supabase
                .from(table)
                .select('id, title')
