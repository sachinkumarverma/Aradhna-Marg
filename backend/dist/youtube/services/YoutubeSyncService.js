        ...
        try {
            ...
            for (const video of videos) {
                ...
                if (existing) {
                    ...
                    const { data: newBhajan, error } = await supabase_1.supabase
                        .from('bhajans')
                        .insert({
                        ...metadata,
                        slug,
            ...
            if (!publishedAfter && fetchedVideoIds.length > 0) {
                const { data: activeVideos } = await supabase_1.supabase
                    .from('bhajans')
            ...
            logger_1.logger.success(`YouTube Sync completed. Imported: ${importedCount}, Updated: ${updatedCount}`);
            // Log sync history
