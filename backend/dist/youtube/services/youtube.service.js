...
const YoutubeVideoRepository_1 = require("../repositories/YoutubeVideoRepository");
...
class YoutubeService {
    async getVideos(search, status, type, sortBy = 'published_at', sortOrder = 'desc', page = 1, limit = 20) {
        return await YoutubeVideoRepository_1.youtubeVideoRepository.getVideos(search, status, type, sortBy, sortOrder, page, limit);
    }
    async getStats() {
        const dbStats = await YoutubeVideoRepository_1.youtubeVideoRepository.getStats();
        let channelTotal = 0;
        let channelTitle = '';
        ...
        try {
            ...
            if (settings?.youtubeChannelId && apiKey) {
                const res = await axios_1.default.get(`https://www.googleapis.com/youtube/v3/channels?key=${apiKey}&id=${settings.youtubeChannelId}&part=statistics,snippet`);
    ...
    async getSyncHistory() {
        return await YoutubeVideoRepository_1.youtubeVideoRepository.getSyncHistory();
    }
    async syncNow() {
        const settings = await SettingsRepository_1.settingsRepository.getSettings();
        ...
        try {
            ...
            do {
                ...
                // Upsert to DB
                const result = await YoutubeVideoRepository_1.youtubeVideoRepository.upsertVideos(videosToUpsert);
            ...
            if (settings.id) {
                await SettingsRepository_1.settingsRepository.updateSettings(settings.id, {
                    youtubeLastSync: new Date().toISOString()
                });
            }
            await YoutubeVideoRepository_1.youtubeVideoRepository.logSync(channelId, 'COMPLETED', `Imported: ${totalImported}, Updated: ${totalUpdated}`);
            return { imported: totalImported, updated: totalUpdated };
        }
        catch (error) {
            console.error('YouTube API Sync Error:', error.response?.data || error.message);
            await YoutubeVideoRepository_1.youtubeVideoRepository.logSync(channelId, 'FAILED', error.message);
            throw new appError_1.AppError('Failed to synchronize with YouTube API', 500);
        }
    }
    async linkBhajan(videoId, bhajanId) {
        return await YoutubeVideoRepository_1.youtubeVideoRepository.linkBhajan(videoId, bhajanId);
    }
    async updateStatus(videoId, status) {
        return await YoutubeVideoRepository_1.youtubeVideoRepository.updateStatus(videoId, status);
    }
    async deleteVideo(videoId) {
        return await YoutubeVideoRepository_1.youtubeVideoRepository.deleteVideo(videoId);
    }
}
