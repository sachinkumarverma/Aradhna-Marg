...
class YoutubeController {
    async getVideos(req, res, next) {
        try {
            ...
            const { data, total } = await youtube_service_1.youtubeService.getVideos(search, status, type, sortBy, sortOrder, page, limit);
    ...
    async getStats(req, res, next) {
        try {
            const stats = await youtube_service_1.youtubeService.getStats();
    ...
    async getSyncHistory(req, res, next) {
        try {
            const history = await youtube_service_1.youtubeService.getSyncHistory();
    ...
    async syncNow(req, res, next) {
        try {
            const result = await youtube_service_1.youtubeService.syncNow();
    ...
    async linkBhajan(req, res, next) {
        try {
            ...
            const result = await youtube_service_1.youtubeService.linkBhajan(id, bhajanId || null);
    ...
    async updateStatus(req, res, next) {
        try {
            ...
            const result = await youtube_service_1.youtubeService.updateStatus(id, status);
    ...
    async deleteVideo(req, res, next) {
        try {
            const { id } = req.params;
            const result = await youtube_service_1.youtubeService.deleteVideo(id);
            return (0, apiResponse_1.sendSuccess)(res, 'Video deleted successfully', result);
        }
        catch (error) {
            next(error);
        }
